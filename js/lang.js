...
const translations = {
  ja: {
    ...
    signup_pet_breed: '犬種',
    breed_blank: '犬種を選択',
    breed_toy_poodle: 'トイ・プードル',
    breed_chihuahua: 'チワワ',
    breed_mix_small: '混血犬（体重10kg未満）',
    breed_shiba: '柴',
    breed_miniature_dachshund: 'ミニチュア・ダックスフンド',
    breed_pomeranian: 'ポメラニアン',
    breed_miniature_schnauzer: 'ミニチュア・シュナウザー',
    breed_yorkshire_terrier: 'ヨークシャー・テリア',
    breed_french_bulldog: 'フレンチ・ブルドッグ',
    breed_maltese: 'マルチーズ',
    breed_shih_tzu: 'シー・ズー',
    breed_kaninchen_dachshund: 'カニーンヘン・ダックスフンド',
    breed_papillon: 'パピヨン',
    breed_golden_retriever: 'ゴールデン・レトリーバー',
    breed_welsh_corgi: 'ウェルシュ・コーギー・ペンブローク',
    breed_jack_russell: 'ジャック・ラッセル・テリア',
    breed_labrador_retriever: 'ラブラドール・レトリーバー',
    breed_pug: 'パグ',
    breed_cavalier_king_charles: 'キャバリア・キング・チャールズ・スパニエル',
    breed_miniature_pinscher: 'ミニチュア・ピンシャー',
    breed_mix_medium: '混血犬（体重10kg以上20kg未満）',
    breed_pekingese: 'ペキニーズ',
    breed_italian_greyhound: 'イタリアン・グレーハウンド',
    breed_border_collie: 'ボーダー・コーリー',
    breed_beagle: 'ビーグル',
    breed_bichon_frise: 'ビション・フリーゼ',
    breed_shetland_sheepdog: 'シェットランド・シープドッグ',
    breed_boston_terrier: 'ボストン・テリア',
    breed_american_cocker_spaniel: 'アメリカン・コッカー・スパニエル',
    breed_japanese_spitz: '日本スピッツ',
    ph_name: '山田太郎',
    ph_furigana: 'やまだたろう',
  ...
  en: {
    ...
    pet_other: 'Other',
    signup_pet_breed: 'Breed',
    breed_blank: 'Select breed',
    breed_toy_poodle: 'Toy Poodle',
    breed_chihuahua: 'Chihuahua',
    breed_mix_small: 'Mixed breed (<10kg)',
    breed_shiba: 'Shiba Inu',
    breed_miniature_dachshund: 'Miniature Dachshund',
    ...
    breed_maltese: 'Maltese',
    breed_shih_tzu: 'Shih Tzu',
    breed_kaninchen_dachshund: 'Kaninchen Dachshund',
    breed_papillon: 'Papillon',
    breed_golden_retriever: 'Golden Retriever',
    breed_welsh_corgi: 'Welsh Corgi Pembroke',
    breed_jack_russell: 'Jack Russell Terrier',
    breed_labrador_retriever: 'Labrador Retriever',
    breed_pug: 'Pug',
    breed_cavalier_king_charles: 'Cavalier King Charles Spaniel',
    breed_miniature_pinscher: 'Miniature Pinscher',
    breed_mix_medium: 'Mixed breed (10-20kg)',
    breed_pekingese: 'Pekingese',
    breed_italian_greyhound: 'Italian Greyhound',
    breed_border_collie: 'Border Collie',
    breed_beagle: 'Beagle',
    breed_bichon_frise: 'Bichon Frise',
    breed_shetland_sheepdog: 'Shetland Sheepdog',
    breed_boston_terrier: 'Boston Terrier',
    breed_american_cocker_spaniel: 'American Cocker Spaniel',
    breed_japanese_spitz: 'Japanese Spitz',
    ph_name: 'Taro Yamada',
...
function applyTranslations() {
  ...
  // ページ固有の追加更新: ペット種別や犬種セレクトなどを言語変更時に更新します。
  try {
    if (typeof window.updateBreedOptions === 'function') {
      window.updateBreedOptions();
    }
    if (typeof window.updateProfilePets === 'function') {
      window.updateProfilePets();
    }
  } catch (_) {
    /* ignore */
  }
...
function cycleLang() {
  ...
  // これにより、地図上のラベルも選択した言語に切り替わります。WordPress の /map/ パーマリンクにも対応します。
  try {
	  // (/map/ または map.html のとき) Google Maps の language/region を反映させるため
	  // ?gm_lang=<ja|en|zh|ko> を付与して再遷移（既存クエリは維持して置換）
	  const path = (window.location.pathname || '').toLowerCase();
	  const isStatic = path.endsWith('map.html');
	  const isWpMap  = /\/map\/?$/.test(path);
	  if (isStatic || isWpMap) {
	    const u = new URL(window.location.href);
	    u.searchParams.set('gm_lang', next);
